...
- **미니맵**: 오른쪽 하단, 클릭으로 위치 이동, 드래그로 부드럽게 뷰포트 이동, 선택된 노드 파란색 하이라이트, 접힌 서브플로우 표시
...
- **복사/붙여넣기**: Ctrl+C / Ctrl+V (Cmd+C / Cmd+V), 그룹 포함, 뷰포트 중앙에 붙여넣기
...
- **서브플로우 패널**: 서브플로우 선택 시 좌측에 포트 편집 UI 표시 (포트 이름 변경, 제거)
- **템플릿 시스템**: T 키로 템플릿 브라우저 열기, 서브플로우를 템플릿으로 저장/재사용
